...
class Solution {
private:
    Node* constructBST(int pre[], int size, int &index, int minRange, int maxRange) {
        if(index == size || pre[index] > maxRange || pre[index] < minRange)
            return NULL;
        Node* root =  newNode(pre[index++]);
        root->left = constructBST(pre, size, index, minRange, root->data);
        root->right = constructBST(pre, size, index, root->data, maxRange);
...
public:
    ...
    Node* post_order(int pre[], int size) {
        ...
        int index = 0;
        Node* root = constructBST(pre,size, index, minRange, maxRange);
